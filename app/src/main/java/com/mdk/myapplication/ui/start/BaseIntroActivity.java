...
public class BaseIntroActivity extends BaseActivity {
    private final String TAG = BaseIntroActivity.class.getName();
        ...
        new HttpHelper.IntroScreenRequest(this, new IntroScreenItemCallback() {
            ...
            public void onSuccess(List<IntroScreenItem> introScreenItemList) {
                ...
                for(int i=0;i<introScreenItemList.size();i++){
                    ...
                    if(i == 0){
                        fragmentArrayList.add(FirstIntroFragment.fragmentOne(introScreenItem.getImage_path(),introScreenItem.getImage_back_url(),introScreenItem.getTitle(),introScreenItem.getExp()));
                    }
                    if(i == 1){
                        fragmentArrayList.add(SecondIntroFragment.fragmentTwo(introScreenItem.getImage_path(),introScreenItem.getImage_back_url(),introScreenItem.getTitle(),introScreenItem.getExp()));

                    }
                    if(i == 2){
                        fragmentArrayList.add(ThirdIntroFragment.fragmentThird(introScreenItem.getImage_path(),introScreenItem.getImage_back_url(),introScreenItem.getTitle(),introScreenItem.getExp()));

                    }
                    if(i == 3){
                        fragmentArrayList.add(FourthIntroFragment.fragmentFour(introScreenItem.getImage_path(),introScreenItem.getImage_back_url(),introScreenItem.getTitle(),introScreenItem.getExp()));
